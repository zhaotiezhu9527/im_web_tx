...
export default class TUIContactServer extends IComponentServer {
  ...
  private bindTIMEvent() {
    ...
    this.TUICore.tim.on(
      ...
      this.handleFriendListUpdated,
      this
    )
    this.TUICore.tim.on(
      this.TUICore.TIM.EVENT.USER_STATUS_UPDATED,
      this.handleUserStatusUpdated,
      this
    )
  }
  ...
  private handleUserStatusUpdated(event: any) {
    ...
    userStatusList.forEach((item: { userID?: string; statusType?: number; customStatus?: any }) => {
      ...
      this.TUICore?.TUIServer?.TUIConversation?.currentStore?.userStatusList?.set(userID, {
        statusType,
        customStatus
      })
    })
  ...
  private handleFilterSystem(list: any) {
    ...
    options.systemConversationList = list.filter(
    ...
    return this.handlePromiseCallback(async (resolve: any, reject: any) => {
      try {
        ...
        this.currentStore.userIDList =
          this.currentStore.friendList.map((item: any) => item.userID) || []
        resolve(imResponse)
      } catch (error) {
        reject(error)
      }
